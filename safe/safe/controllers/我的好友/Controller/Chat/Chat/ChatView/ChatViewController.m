...
- (void)setupBarButtonItem
{
    ...
    if (self.isChatGroup) {
        ...
        [detailButton setImage:[UIImage imageNamed:@"groupDetail"] forState:UIControlStateNormal];
        ...
        [clearButton setImage:[UIImage imageNamed:@"gerenwu"] forState:UIControlStateNormal];
        [clearButton addTarget:self action:@selector(showChatterDetail) forControlEvents:UIControlEventTouchUpInside];
...
//                        }
                        cellModel.headImageURL = [NSURL URLWithString:_rbHeadImgUrl];
...
//            }
            if ([model.username isEqualToString:_chatter]) {
                model.headImageURL = [NSURL URLWithString: _rbHeadImgUrl];
            }else
            {
                model.headImageURL = [NSURL URLWithString:_myHeadImgUrl];
            }
            //Demo集成Parse,获取用户个人信息
//            UserProfileEntity *user = [[UserProfileManager sharedInstance] getUserProfileByUsername:model.username];
//            
...
-(void)showChatterDetail
{
    DbgLog(@"当前聊天的人是:%@",self.chatter);
    FriendDetailViewController *fdVC = [[FriendDetailViewController alloc]initWithNibName:@"FriendDetailViewController" bundle:nil];
    fdVC.chatterName = self.chatter;
    fdVC.rauth = self.rauth;
