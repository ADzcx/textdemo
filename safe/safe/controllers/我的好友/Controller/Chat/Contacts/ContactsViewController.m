...
@interface ContactsViewController ()<UITableViewDataSource, UITableViewDelegate, UISearchBarDelegate, UISearchDisplayDelegate, UIActionSheetDelegate, SRRefreshDelegate>
{
...
@property (strong, nonatomic) NSMutableDictionary *ourUserDetailDic;
@property (strong, nonatomic) NSMutableArray *ourUserArray;
...
- (void)viewDidLoad
{
    ...
    _ourUserArray = [[NSMutableArray alloc]init];
...
//    self.tableView.frame = CGRectMake(0, self.searchBar.frame.size.height, self.view.frame.size.width, self.view.frame.size.height - self.searchBar.frame.size.height);
    ...
    self.tableView.frame = CGRectMake(0, self.searchBar.frame.size.height, self.view.frame.size.width, self.view.frame.size.height+20);
...
-(void)prepareOurUser
{
    ...
    [manager POST:strUrl parameters:param success:^(AFHTTPRequestOperation *operation, id responseObject) {
        ...
        for (NSDictionary *userInfoDic  in friendsArray) {
            ...
            [_ourUserArray addObject:[NSString stringWithFormat:@"%@",userInfoDic[@"rbid"]]];
        }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.row == 0) {
        ...
        cell.indexPath = indexPath;
        if (_ourUserArray.count > 0) {
            NSString *FuserId = _ourUserArray[indexPath.row];
            DbgLog(@"取出的userId ＝ %@",FuserId);
            cell.username = [self want:1 who:FuserId];
            
            [cell.imageView sd_setImageWithURL:[NSURL URLWithString:[self want:2 who:FuserId]] placeholderImage:[UIImage imageNamed:@"icon_logo"]];
        }
    }
    
    return cell;
}
-(void)getUserHeadAndName
{
...
//        ApplyEntity *entity = [self.dataSource objectAtIndex:indexPath.row];
        ...
        NSUserDefaults *userdef = [NSUserDefaults standardUserDefaults];
        NSDictionary *userInfo = [userdef valueForKey:@"info"];
        NSMutableDictionary *param = [NSMutableDictionary dictionaryWithObjectsAndKeys:@"token",userInfo[@"token"], nil];
        [param setValue:buddy.username forKey:@"userIdB"];

        DbgLog(@"send para:%@",param);
        NSString *url = [NSString stringWithFormat:@"%@%@",HeadUrl,@"/friend/del"];
        [[AFHTTPSessionManager manager] GET:url parameters:param success:^(NSURLSessionDataTask *task, id responseObject) {
            DbgLog(@"%@",responseObject);
            NSInteger statue = [responseObject[@"code"] integerValue];
            if (statue == 200) {
                [self showHint:@"删除成功"];
...
//            }
//        }
        NSString *userId = _ourUserArray[indexPath.row];
        
        
        ChatViewController *chatVC = [[ChatViewController alloc] initWithChatter:userId isGroup:NO];
        
//        chatVC.title = buddy.username;
        chatVC.title = [self want:1 who:userId];
