...
@property (nonatomic,copy)NSMutableDictionary *ourUserDetailDic;
@property (nonatomic,copy)NSMutableArray *ourUserArray;
...
-(void)prepareOurUser
{
    ...
    [manager POST:strUrl parameters:param success:^(AFHTTPRequestOperation *operation, id responseObject) {
        ...
        [_ourUserDetailDic removeAllObjects];
        [_ourUserArray removeAllObjects];
...
-(UITableViewCell *)tableView:(UITableView *)tableView
    ...
    if (conversation.conversationType == eConversationTypeChat) {
        if (_ourUserArray.count>0) {
            ...
            cell.name = [self want:1 who:conversation.chatter];
            cell.imageURL = [NSURL URLWithString:[self want:2 who:conversation.chatter]];
        }else
        {
            cell.name = [[RobotManager sharedInstance] getRobotNickWithUsername:conversation.chatter];
            cell.placeholderImage = [UIImage imageNamed:@"icon_logo"];
        }
    }
    else{
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (conversation.conversationType != eConversationTypeChat) {
        ...
        if ([self want:1 who:conversation.chatter]) {
            
        }
        title = [self want:1 who:conversation.chatter];
...
//        title = [[UserProfileManager sharedInstance] getNickNameWithUsername:conversation.chatter];
    ...
    if ([[RobotManager sharedInstance] isRobotWithUsername:chatter]) {
        ...
        if(_ourUserArray.count > 0 ){
            chatController.rbHeadImgUrl = [self want:2 who:chatter];
            chatController.myHeadImgUrl = _myHeadImage;
        }
    }
