...
-(instancetype)initDatePickWithDate:(NSDate *)defaulDate datePickerMode:(UIDatePickerMode)datePickerMode isFromeNow:(BOOL)fromeNoew isHaveNavControler:(BOOL)isHaveNavControler{
...
#pragma mark －－－这里设置时区
    ...
    datePicker.backgroundColor=[UIColor colorWithWhite:0.9 alpha:1];
    if (_defaulDate) {
...
#pragma mark －－－结果还是格林尼治时间
        DbgLog(@"选择器里的时间是:-->%@",_datePicker.date);
        NSDate *getDate = _datePicker.date;
        
        NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
        [dateFormatter setDateFormat:@"yyyy-MM-dd'T'HH:mm:ssZ"];
        
        NSDate* destinationDateNow = [self getNowDateFromatAnDate:getDate];
        
        _resultString=[NSString stringWithFormat:@"%@",destinationDateNow];
